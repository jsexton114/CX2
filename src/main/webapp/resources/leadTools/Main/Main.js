    ...
    (function (DocumentViewerDemo) {
        ...
        var DocumentViewerDemoApp = (function () {
            ...
            DocumentViewerDemoApp.prototype.updateContainers = function () {
                var headerToolbarContainerHeight = $(this.headerToolbarContainer).height();
                var footerToolbarContainerHeight = this.demoMode == DemoMode.Default ? $(this.footerToolbarContainer).height() : 0;
                // Check visibility
                var visibleThumbnails = $(this.thumbnailsContainer).is(":visible");
                var visibleBookmarks = $(this.bookmarksContainer).is(":visible");
                var visibleAnnotationsListControls = $(this.annotationsListControlsContainer).is(":visible");
                ...
                var navigationbarContainerWidth = DocumentViewerDemoApp.isMobileVersion ? 0 : $(this.navigationbarContainer).width();
                ...
                $.getJSON("./serviceConfig.json", { _: new Date().getTime() })
                    ...
                    lt.Documents.DocumentFactory.verifyService()
                        ...
                        if (!response.isLicenseChecked) {
                            ...
                            console.error("Warning! The LEADTOOLS License used in the service could not be found. This demo may not function as expected.");
                        }
                            ...
                            console.error("Warning! The LEADTOOLS Kernel has expired. This demo may not function as expected.");
                        }
                        if (!response.isCacheAccessible) {
                            ...
                            console.error("Warning! The server's cache directory does not exist or cannot be written to. This demo may not function as expected.");
                        }
                        ...
                        console.log("Loading initial document from '" + defaultDocument + "'. If this is the incorrect URL, check your values in serviceConfig.json");
                ...
                if (this.useElements) {
                    ...
                    if (lt.LTHelper.supportsCSSTransitions) {
                        ...
                        var stopTransitionListening = function () {
                            ...
                            imageViewer.viewDiv.removeEventListener("transitionend", transitionEndCallback, false);
                            lt.LTHelper.removeClass(imageViewer.foreCanvas, canvasTransitionClass);
                        };
                        ...
                        imageViewer.elementsUpdated.add(function (sender, e) {
                            if (!e.isTransitionsEnabled) {
                                ...
                                imageViewer.viewDiv.addEventListener("transitionend", transitionEndCallback, false);
                                lt.LTHelper.addClass(imageViewer.foreCanvas, canvasTransitionClass);
                            }
                        });
                    }
                    var checkForError = function (sender, e) {
                        ...
                        if (itemIndex > -1 && _this._elementsItemErrors.indexOf(itemIndex) > -1) {
                            $(item.itemElement).addClass("image_error");
                        }
                    };
                    imageViewer.autoItemElementsAdded.add(checkForError.bind(this));
                }
            ...
            DocumentViewerDemoApp.showServiceError = function (message, jqXHR, statusText, errorThrown) {
                ...
                if (!serviceError.isParseError && !serviceError.isBrowserError) {
                    ...
                    if (serviceError.link) {
                        ...
                        console.error("Service Error - Help Link:", serviceError.link, serviceError);
                    }
                    else
            ...
            DocumentViewerDemoApp.prototype.convertDocument = function (jobData) {
                ...
                prepareToSavePromise.fail(function (jqXHR, statusText, errorThrown) {
                    _this.endBusyOperation();
            ...
            DocumentViewerDemoApp._barcodeSymbologyNames = [
                ...
                "PDF417", "MicroPDF417", "Datamatrix", "QR", "Aztec", "Maxi", "MicroQR"
            ];
