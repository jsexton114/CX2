...
public class FormService {
	...
	public String getSigningDocumentResponse(String formGuid, String formTitle, String creatorFullName, String fieldDataJsonString, String municipalityLogo, String clientId, String clientSecret, String firstNameOfRecipientParty, String lastNameOfRecipientParty, String emailIdOfRecipientParty) throws IOException,
	java.sql.SQLException {
		...
		Map < String,
		Object > formData = getFormDataByLabel(formGuid);
		...
		folderResponsePojo = ESignGenieApi.createSigningDocument(formDataPojo, formTitle, formData, testLogo, clientId, clientSecret, firstNameOfRecipientParty, lastNameOfRecipientParty, emailIdOfRecipientParty);
	...
	private Map < String,
	Object > getFormDataByLabel(String formGuid) throws SQLServerException,
	SQLException {
		// NOTE: all DB code should be refactorred out to a data abstraction layer class that doesn't expose vendor DB references or SQL references
		DBQueryParams params = new DBQueryParams();
		Connection cx2Conn = DBConnectionService.getConnection();
		...
		Map < String,
		Object > rawFormData = getFormData(formGuid);

		Map < String,
		Object > formData = new HashMap < String,
		Object > ();
	...
	private UserDataPojo getUserData() throws SQLServerException,
	SQLException {
	...
	private Boolean userIsEmployee(Connection cx2Conn, String formGuid) throws SQLException {
		...
		Boolean userIsEmployee = DBUtils.selectQuery(cx2Conn, "SELECT count(*) AS employeeRoleCount FROM Roles R " + "WHERE R.UserId=:userId " + "AND " + "R.RoleName='MunicipalityEmployee' AND R.MunicipalityId=(SELECT FT.MunicipalityId FROM MasterForms MF, FormTypes FT WHERE MF.FormGUID=:formGuid AND FT.ID=MF.FormTypeId)", queryParams).get(0).getInteger("employeeRoleCount") > 0;;
	...
	private Boolean userIsAdmin(Connection cx2Conn, String formGuid) throws SQLException {
		...
		Boolean userIsAdmin = DBUtils.selectQuery(cx2Conn, "SELECT count(*) AS adminRoleCount FROM Roles R " + "WHERE R.UserId=:userId " + "AND ( " + "	R.RoleName='CXAdmin' " + "	OR (R.RoleName='MunicipalityAdmin' AND R.MunicipalityId=(SELECT FT.MunicipalityId FROM MasterForms MF, FormTypes FT WHERE MF.FormGUID=:formGuid AND FT.ID=MF.FormTypeId))" + ")", queryParams).get(0).getInteger("adminRoleCount") > 0;
	...
	private Boolean userCanEdit(Connection cx2Conn, String formGuid) throws SQLException {
		...
		// Write group
		Boolean queryResult = DBUtils.selectQuery(cx2Conn, "select count(*) AS isInWriteGroup from MasterForms MF, FormStatuses FS, MunicipalityGroups MG " + "WHERE MF.FormGUID=:FormGUID " + "and FS.ID=MF.formStatusId " + "and MG.ID=FS.WriteAccess " + "and :UserId IN (SELECT GM.userId FROM MunicipalityGroupMembers GM WHERE GM.municipalityGroupId=MG.id)", queryParams).get(0).getLong("isInWriteGroup") > 0;
	...
	private Boolean userIsProcessOwner(Connection cx2Conn, String formGuid) throws SQLException {
		...
		DBRow queryResult = DBUtils.selectQuery(cx2Conn, "select count(*) AS IsProcessOwner from MasterForms MF, FormStatuses FS, MunicipalityGroups MG " + "WHERE MF.FormGUID=:FormGUID " + "and FS.ID=MF.formStatusId " + "and MG.ID=FS.processOwners " + "and :UserId IN (SELECT GM.userId FROM MunicipalityGroupMembers GM WHERE GM.municipalityGroupId=MG.id)", queryParams).get(0);
	...
	public Map < String,
	Object > getFormData(String formGuid) throws SQLException {
		Connection cx2Conn = DBConnectionService.getConnection();
	...
	public Long saveFormType(Long municipalityId, String formType, String description) throws SQLException {
		...
		try {
			...
			DBUtils.simpleQuery(cx2Conn, "INSERT INTO FormStatuses (FormTypeId, ConsiderClosed, SortOrder, Status, Description, SendEmail) " + "VALUES (:newFormTypeId, 0, 1, 'Submitted', 'Submitted', 1)", formCreateParams);

			DBUtils.simpleUpdateQuery(muniDbConn, "CREATE TABLE " + formTableName + " (" + "ID numeric(10) identity(1,1), " + "FormGUID uniqueidentifier NOT NULL DEFAULT NEWSEQUENTIALID(), " + "FormTitle varchar(255), " + "CreatedBy varchar(255), " + "CreatedDate datetime2 NOT NULL DEFAULT sysdatetime(), " + "ModifiedDate datetime2, " + "ModifiedBy varchar(255), " + "TotalSqft numeric(36,2), " + "TotalUnits numeric(36,2), " + "Basement bit, " + "VendorId numeric(10)" + ")");

			DBUtils.simpleQuery(cx2Conn, "INSERT INTO FormTypeFields " + "(FormTypeId, FieldName, Label, DisplayOrder, Required, DefaultValue, FieldTypeId)" + " VALUES (:newFormTypeId, 'TotalSqft', 'Total Square Feet', 100, 1, 0, 5)," + " (:newFormTypeId, 'TotalUnits', 'Total Units', 101, 1, 1, 5)," + " (:newFormTypeId, 'Basement', 'Has Basement', 102, 1, 0, 6)", formCreateParams);
	...
	private String createForm(Connection cx2Conn, Long formTypeId, Long primaryVendorId, Long createUserId, Long ownerId, Long incomplete, Long ownerCompletingWork) throws SQLException {
		DBQueryParams queryParams = new DBQueryParams();
		queryParams.addLong("incomplete", incomplete);
		queryParams.addLong("ownerCompletingWork", ownerCompletingWork);
		queryParams.addLong("formTypeId", formTypeId);
	...
	private void uploadDocuments(Connection cx2Conn, MultipartFile[] files, String formGuid, Boolean isNew) throws SQLException,
	IOException {
	...
	public void sendLetter(String formGuid, Integer letterTemplateId, String formLink) throws SQLException,
	MessagingException {
		Connection cx2Conn = DBConnectionService.getConnection();
	...
	private byte[] createLetterPdf(Integer letterTemplateId, Long formTypeId, String formGuid) throws SQLException {
		...
		Map < String,
		String > textTokens = LetterTemplate.getTextTokenValues(db, formTypeId, formGuid);
		byte[] fileBytes = lt.createLetter(globalFormInfo, textTokens);

		return fileBytes;
	}

	private void sendStatusUpdateMail(String formGuid, Long formStatusId, String formLink) throws MessagingException,
	SQLException {
		Connection cx2Conn = DBConnectionService.getConnection();
		Boolean sendEmail;
	...
	private void sendOnZeroStatusUpdateMail(String formGuid, Long formStatusId, String formLink, Connection cx2Conn) throws MessagingException,
	SQLException {
	...
	public void setFormStatus(String formGuid, Long formStatusId, Long incomplete, String comments, String formLink) throws SQLException,
	MessagingException {

		setFormStatus(formGuid, formStatusId, incomplete, comments);
		sendStatusUpdateMail(formGuid, formStatusId, formLink);
	}

	public void setOnZeroFormStatus(String formGuid, Long formStatusId, String comments, String formLink, Connection cx2Conn) throws SQLException,
	MessagingException {
		setOnZeroFormStatus(formGuid, formStatusId, comments, cx2Conn);
		sendOnZeroStatusUpdateMail(formGuid, formStatusId, formLink, cx2Conn);
	}

	private void setFormStatus(String formGuid, Long formStatusId, Long incomplete, String comments) throws SQLException {
		Connection cx2Conn = DBConnectionService.getConnection();
		...
		try {
			...
			queryParams.addLong("newIncompleteFlag", incomplete);
			...
			long oldIncompleteFlag = masterFormData.getBoolean("Incomplete") ? 1 : 0;
			queryParams.addLong("oldIncompleteFlag", oldIncompleteFlag);

			if (oldIncompleteFlag != incomplete) {
				String flagComments = (incomplete == 1) ? "The form is no longer considered incomplete.": "This form has been flagged as incomplete.";
				queryParams.addString("flagComments", flagComments);
				DBUtils.simpleUpdateQuery(cx2Conn, "INSERT INTO FormHistory (FormGUID,FormTypeId,NewStatusId,OldStatusId,Comments,CreatedBy,CreatedTime) " + "VALUES (:formGuid,:FormTypeId,:newFormStatusId,:OldStatusId,:flagComments,:CreatedBy,SYSUTCDATETIME())", queryParams);
			}

			DBRow formTypeData = DBUtils.selectQuery(cx2Conn, "SELECT * FROM FormTypes WHERE ID=:FormTypeId", queryParams).get(0);
			DBUtils.simpleUpdateQuery(cx2Conn, "UPDATE MasterForms SET Incomplete=:newIncompleteFlag, FormStatusId=:newFormStatusId," + "Closed=(SELECT ConsiderClosed FROM FormStatuses WHERE ID=:newFormStatusId) WHERE FormGUID=:formGuid", queryParams);

			DBUtils.simpleUpdateQuery(cx2Conn, "INSERT INTO FormHistory (FormGUID,FormTypeId,NewStatusId,OldStatusId,Comments,CreatedBy,CreatedTime) " + "VALUES (:formGuid,:FormTypeId,:newFormStatusId,:OldStatusId,:Comments,:CreatedBy,SYSUTCDATETIME())", queryParams);
	...
	private void setOnZeroFormStatus(String formGuid, Long formStatusId, String comments, Connection cx2Conn) throws SQLException {
		...
		try {
			...
			DBUtils.simpleUpdateQuery(cx2Conn, "UPDATE MasterForms SET FormStatusId=:newFormStatusId, " + "Closed=(SELECT ConsiderClosed FROM FormStatuses WHERE ID=:newFormStatusId) WHERE FormGUID=:formGuid", queryParams);
	...
	public String submitForm(Long incomplete, Long ownerCompletingWork, Long formTypeId, Long behalfOfUserId, Long ownerId, String locationIds, String vendorIds, Long primaryVendorId, String usersWithWhomToShare, String fieldDataJsonString, Long draftId, MultipartFile[] attachments) throws Exception {
		Connection cx2Conn = DBConnectionService.getConnection();
		cx2Conn.setAutoCommit(false);
		String formGuid = "";
		...
		Gson gson = gb.create();
		HashMap < String,
		Object > fieldData = new HashMap < String,
		Object > ();
		Type genericType = new TypeToken < HashMap < String,
		Object >> () {}.getType();
		...
		try {
			...
			formGuid = createForm(cx2Conn, formTypeId, primaryVendorId, createUserId, ownerId, incomplete, ownerCompletingWork);
			...
			DBUtils.simpleUpdateQuery(cx2Conn, "INSERT INTO FormHistory " + "(FormGUID, FormTypeId, NewStatusId, CreatedBy, CreatedTime) " + "VALUES (:formGuid, :formTypeId, :newFormStatusId, :createUserId, :createdTime)", queryParams);
