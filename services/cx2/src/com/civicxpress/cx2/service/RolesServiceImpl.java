...
public class RolesServiceImpl implements RolesService {
    ...
    @Transactional(readOnly = true, value = "cx2TransactionManager")
    @Override
    public Roles getByRoleNameAndMunicipalityIdAndUserId(String roleName, Integer municipalityId, Integer userId) {
        Map<String, Object> roleNameAndMunicipalityIdAndUserIdMap = new HashMap<>();
        roleNameAndMunicipalityIdAndUserIdMap.put("roleName", roleName);
        roleNameAndMunicipalityIdAndUserIdMap.put("municipalityId", municipalityId);
        roleNameAndMunicipalityIdAndUserIdMap.put("userId", userId);

        LOGGER.debug("Finding Roles by unique keys: {}", roleNameAndMunicipalityIdAndUserIdMap);
        Roles roles = this.wmGenericDao.findByUniqueKey(roleNameAndMunicipalityIdAndUserIdMap);

        if (roles == null){
            LOGGER.debug("No Roles found with given unique key values: {}", roleNameAndMunicipalityIdAndUserIdMap);
            throw new EntityNotFoundException(String.valueOf(roleNameAndMunicipalityIdAndUserIdMap));
        }
