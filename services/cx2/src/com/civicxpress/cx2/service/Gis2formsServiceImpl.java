...
public class Gis2formsServiceImpl implements Gis2formsService {
    ...
    @Transactional(readOnly = true, value = "cx2TransactionManager")
    @Override
    public Gis2forms getByGisrecordIdAndRelatedFormGuid(Integer gisrecordId, String relatedFormGuid) {
        Map<String, Object> gisrecordIdAndRelatedFormGuidMap = new HashMap<>();
        gisrecordIdAndRelatedFormGuidMap.put("gisrecordId", gisrecordId);
        gisrecordIdAndRelatedFormGuidMap.put("relatedFormGuid", relatedFormGuid);

        LOGGER.debug("Finding Gis2forms by unique keys: {}", gisrecordIdAndRelatedFormGuidMap);
        Gis2forms gis2forms = this.wmGenericDao.findByUniqueKey(gisrecordIdAndRelatedFormGuidMap);

        if (gis2forms == null){
            LOGGER.debug("No Gis2forms found with given unique key values: {}", gisrecordIdAndRelatedFormGuidMap);
            throw new EntityNotFoundException(String.valueOf(gisrecordIdAndRelatedFormGuidMap));
        }
