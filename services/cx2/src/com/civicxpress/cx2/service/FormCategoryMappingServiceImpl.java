...
public class FormCategoryMappingServiceImpl implements FormCategoryMappingService {
    ...
    @Transactional(readOnly = true, value = "cx2TransactionManager")
    @Override
    public FormCategoryMapping getByFormTypeIdAndFormCategoryId(Integer formTypeId, Integer formCategoryId) {
        Map<String, Object> formTypeIdAndFormCategoryIdMap = new HashMap<>();
        formTypeIdAndFormCategoryIdMap.put("formTypeId", formTypeId);
        formTypeIdAndFormCategoryIdMap.put("formCategoryId", formCategoryId);

        LOGGER.debug("Finding FormCategoryMapping by unique keys: {}", formTypeIdAndFormCategoryIdMap);
        FormCategoryMapping formCategoryMapping = this.wmGenericDao.findByUniqueKey(formTypeIdAndFormCategoryIdMap);

        if (formCategoryMapping == null){
            LOGGER.debug("No FormCategoryMapping found with given unique key values: {}", formTypeIdAndFormCategoryIdMap);
            throw new EntityNotFoundException(String.valueOf(formTypeIdAndFormCategoryIdMap));
        }
