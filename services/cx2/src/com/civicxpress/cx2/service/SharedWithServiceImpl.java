...
public class SharedWithServiceImpl implements SharedWithService {
    ...
    @Transactional(readOnly = true, value = "cx2TransactionManager")
    @Override
    public SharedWith getByRelatedGuidAndSharedWithUserAndCreatedBy(String relatedGuid, Integer sharedWithUser, Integer createdBy) {
        Map<String, Object> relatedGuidAndSharedWithUserAndCreatedByMap = new HashMap<>();
        relatedGuidAndSharedWithUserAndCreatedByMap.put("relatedGuid", relatedGuid);
        relatedGuidAndSharedWithUserAndCreatedByMap.put("sharedWithUser", sharedWithUser);
        relatedGuidAndSharedWithUserAndCreatedByMap.put("createdBy", createdBy);

        LOGGER.debug("Finding SharedWith by unique keys: {}", relatedGuidAndSharedWithUserAndCreatedByMap);
        SharedWith sharedWith = this.wmGenericDao.findByUniqueKey(relatedGuidAndSharedWithUserAndCreatedByMap);

        if (sharedWith == null){
            LOGGER.debug("No SharedWith found with given unique key values: {}", relatedGuidAndSharedWithUserAndCreatedByMap);
            throw new EntityNotFoundException(String.valueOf(relatedGuidAndSharedWithUserAndCreatedByMap));
        }
