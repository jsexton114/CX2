...
public class InspectionService {
    ...
    public void scheduleInspection(String formGuid, Long inspectionDesignId, Date requestedFor, String targetLink) throws SQLException,MessagingException {
    	Connection cx2Conn = DBConnectionService.getConnection();
    	...
    	try {
	    	...
	    	// Check for scheduleInspection Mail
	    	requestedInspectionMail(cx2Conn, inspectionTitle.toString(), formGuid, inspectionDesignId, requestedFor, municipalityId, newInspectionGuid,targetLink);	
    ...
    private void requestedInspectionMail(Connection cx2Conn, String inspectionTitle, String formGuid, Long inspectionDesignId, Date requestedFor, Long municipalityId, String newInspectionGuid, String targetLink) throws SQLException, MessagingException {
	DBQueryParams queryParams = new DBQueryParams();
	queryParams.addLong("inspectionDesignId", inspectionDesignId);
	DBRow inspectionOutcomeData = DBUtils.selectOne(cx2Conn, "SELECT * from InspectionOutcome where InspectDesignId=:inspectionDesignId and Outcome='Requested' ", queryParams);
	if (inspectionOutcomeData.getBoolean("SendEmail")) {
		queryParams.addLong("municipalityId", municipalityId);
		queryParams.addString("formGuid", formGuid);
		queryParams.addLong("userId", Long.parseLong(securityService.getUserId()));
		DBRow municipalityData = DBUtils.selectOne(cx2Conn, "SELECT * from Municipalities where ID=:municipalityId ", queryParams);
		String formTitle = DBUtils.selectOne(cx2Conn, "SELECT FormTitle from MasterForms where FormGUID=:formGuid", queryParams).getString("FormTitle");
		DBRow userData = DBUtils.selectOne(cx2Conn, "SELECT * from Users where ID=:userId ", queryParams);
		...
		List<DBRow> inspectorManagerEmails = DBUtils.selectQuery(cx2Conn, "select U.Email from Roles R INNER JOIN Users U on R.UserId=U.ID where R.RoleName='InspectorManager' and  R.MunicipalityId=:municipalityId", queryParams);
        
        if (inspectorManagerEmails != null) {
	        InternetAddress[] sharedWithRecipients = new InternetAddress[inspectorManagerEmails.size()];
	        for (int i = 0; i < inspectorManagerEmails.size(); i++) {
	        	sharedWithRecipients[i] = new InternetAddress(inspectorManagerEmails.get(i).getString("Email"));
	        }
	        
	        message.setRecipients(Message.RecipientType.CC, sharedWithRecipients);
        }

		String emailSubject = inspectionOutcomeData.getString("EmailSubjectLine");
		StringBuilder emailContent = new StringBuilder("Hi "+userData.getString("FullName")+",<br /><br />");
        String formLink=targetLink+"Forms?FormGUID="+formGuid;
        String inspectionLink=targetLink+"ViewInspection?inspectionGuid="+newInspectionGuid;
		emailContent.append("An Inspection as been requested on the Form: ");
		emailContent.append("<a href ='" + formLink + "'>" + formTitle + "</a>");
		emailContent.append("<br />");
		emailContent.append(inspectionOutcomeData.getString("EmailBodyText"));
		emailContent.append("<br />");
		emailContent.append(municipalityData.getString("MunicipalityName"));
		emailContent.append("<br />");
		emailContent.append(inspectionTitle);
		emailContent.append("<br />");
		emailContent.append("<a href ='" + inspectionLink + "'>" + inspectionLink + "</a>");
		emailContent.append("<br /><br />");
		emailContent.append(municipalityData.getString("GlobalEmailSig") + "<br/><br/>");
